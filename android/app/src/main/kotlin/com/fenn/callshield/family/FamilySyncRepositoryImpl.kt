...
class FamilySyncRepositoryImpl @Inject constructor(
    ...
    override suspend fun registerPairing(tokenHash: String, expiresAt: String): Result<Unit> = runCatching {
        val response = apiClient.http.post("$baseUrl/family-pair") {
            contentType(ContentType.Application.Json)
            setBody(PairRequest(token_hash = tokenHash, expires_at = expiresAt))
        }
...
@Serializable private data class PairRequest(val token_hash: String, val expires_at: String)
